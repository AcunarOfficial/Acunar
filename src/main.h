...
static const int COINBASE_MATURITY_ACU = 240;
/** Threshold for nLockTime: below this value it is interpreted as block number, otherwise as UNIX timestamp. */
static const int STAKE_TARGET_SPACING = 90; // block spacing 
...
static const int64 COIN_PREMINE = 1300000000 * COIN;
static const int64 COIN_YEAR_REWARD = 5 * CENT;
static const int64 MAX_POS_REWARD = 25 * COIN;
static const int64 LAST_POW_BLOCK = 22080; // Block height of the last proof of work block
static const int DAILY_BLOCKCOUNT = 960;
static const int YEARLY_BLOCKCOUNT = 350400;
...
static const uint256 hashGenesisBlockOfficial("0x0000030cd3a3640ac32f7a6357ea1b6b61aa2c752849b4f78b7a3c1326efc2a4");
static const uint256 hashGenesisBlockTestNet("0x00001dae82302b07e8e39d4b94c737415644c509b7f76dc16cd64b38a1ad2cd7");
...
public:
    ...
    {
        // acunar: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // acunar: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // acunar: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // acunar: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // acunar: whether transaction is a coinstake
    bool fCoinStake;

    // acunar: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // acunar: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // acunar
            undo.nTime = nTime;            // acunar
        }
        return true;
    }
...
public:
    ...
    // acunar: block signature - signed by coin base txout[0]'s owner
    ...
    // acunar: two types of block: proof-of-work or proof-of-stake
    ...
    // acunar: get max transaction timestamp
    ...
    bool GetCoinAge(uint64& nCoinAge) const; // acunar: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // acunar: entropy bit for stake modifier if chosen by modifier
};
...
public:
    ...
    // (memory only) Total amount of trust score (acunar proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // acunar: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // acunar: proof-of-stake related block index fields
    unsigned int nFlags;  // acunar: block index flags
    enum
    {
